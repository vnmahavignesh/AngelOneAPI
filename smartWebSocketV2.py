...
class SmartWebSocketV2(object):
    ...
    def _on_ping(self, wsapp, data):
        ...
        formatted_timestamp = time.strftime(
            "%d-%m-%y %H:%M:%S", time.localtime(timestamp))
        logger.info(
            f"In on ping function ==> {data}, Timestamp: {formatted_timestamp}")
    ...
    def subscribe(self, correlation_id, mode, token_list):
        ...
        try:
            ...
            if mode == self.DEPTH:
                total_tokens = sum(len(token["tokens"])
                                   for token in token_list)
                quota_limit = 50
    ...
    def _on_error(self, wsapp, error):
        ...
        if self.current_retry_attempt < self.MAX_RETRY_ATTEMPT and not self.CLOSE_CONNECTION:
            ...
            try:
                ...
                logger.error(
                    f"Error occurred during resubscribe/reconnect: {e}")
                if hasattr(self, 'on_error'):
                    self.on_error("Reconnect Error", str(
                        e) if str(e) else "Unknown error")
        else:
            self.close_connection()
            if hasattr(self, 'on_error'):
        ...
        try:
            ...
            if parsed_data["subscription_mode"] == self.SNAP_QUOTE:
                ...
                parsed_data["open_interest_change_percentage"] = self._unpack_data(
                    binary_data, 139, 147, byte_format="q")[0]
            ...
            if parsed_data["subscription_mode"] == self.DEPTH:
                ...
                parsed_data["packet_received_time"] = self._unpack_data(
                    binary_data, 35, 43, byte_format="q")[0]
